...
rand::rand(){
	vanaf = new vakje;
	cout << "Moet het aantal oplossingen geteld worden (j/n)? ";
	cin >> TelAantOpl;
}
...
//Controleert of de rand rotatiesymmetrisch is: return (#zwart == #wit).
bool rand::rotatierand(){
	int zwart = 0, wit = 0;
	vakje* p = vanaf;
	while(p != tot){
		zwart++;
...
void rand::nrand(){
	do{
		...
		if (bevatlb(vanaf, tot))
			return;
	}while((!eerste()  && (TelAantOpl == 'n')) || !rotatierand());
}//rand::nrand
...
class oplossing{
	...
	public:
		...
		void ingesloten();//snelheid: O(2n^2)
		void vul_opp(bool);
		void vul_rand(rand);
		void zoek_vakje();
		void gok();
...
//Geeft het vakje terug dat op de plaats van q zou staan na een 180 graden
//rotatie. (delta_y, delta_x) zijn de 'matrixcoordinaten' beginnend bij (1,1).
vakje* oplossing::rotatiesymmetrie(vakje* q){
	vakje* r = q;
	...
	r = start;
	for(int i = 0; i < hoogte - delta_y; i++)
		r = r->buren[1];
	for(int i = 0; i < breedte - delta_x; i++)
		r = r->buren[0];
	return r;
}

//Kleurt een vakje zwart(true) of wit(false) in.
void oplossing::kleur_in(vakje* p, bool zwart){
	if(!p->bevat_info){
...
//Past de "kleur_in" functie toe op p en zijn rotatiesymmetrische verwant.
void oplossing::vul_in(vakje* p, bool zwart){
	vakje* q = rotatiesymmetrie(p);
	if(!p->bevat_info){
		kleur_in(p, zwart);
		kleur_in(q, !zwart);
	}
...
void oplossing::ingesloten(){
	int aantal_opp = 0;
	niets_geteld(false); //Nu is alles "algeteld" false voor alle vakjes.
	vakje* p = eerste();
	while(!p->algeteld){
		niets_geteld(true);
		aantal_opp++;
...
//te doen.
void oplossing::gok(){
	vakje *p, *q = start, *onthoud = start;
	int afstand = 100000;
	for(int i = 0; i < hoogte; i++){
		p = q;
		for(int j = 0; j < breedte; j++){
			if((i*i + j*j < afstand) && (!p->bevat_info)){
				afstand = i*i + j*j;
				onthoud = p;
			}//if
			p = p->buren[0];
...
//Returnt false desda de schaakbordstelling bevestigt dat deze rand fout is.
bool oplossing::oplosbaar(){
	vakje *q = start, *p;
	int blauwminrood = 0;
	bool blauw = true, blauwtemp; //geeft aan welke kleur de linkerbovenhoek is van 2x2.
	bool geenzwart = true;
	for(int i = 0; i < hoogte - 1; i++){
		blauwtemp = blauw;
		p = q;
		for(int j = 0; j < breedte - 1; j++){
			...
			geenzwart = true;
			for(int k = 0; k < 4; k++){
				if(A[k]->bevat_info && A[k]->info)
					geenzwart = false;
			}
			if(geenzwart){
				blauwminrood = blauwminrood - 1 + 2*blauw;
			}
			p = p->buren[0];
			blauw = !blauw;
		}//for
	...
	while(true){
		...
		while(!fout){
			...
			if(vol()){
				...
				ingesloten();
				if(!fout){
					if(Rand.TelAantOpl == 'n'){
						cout << "Gevonden! (" << hoogte << 'x' << breedte << ")" << endl;
						drukaf();
						cout << "Andere oplossing zoeken? (j/n)?";
						cin >> antwoord;
						if(antwoord == 'n'){
							naarfile();
							return;
		...
		if(Stapel.bovenste != NULL){//De gok moest wit ipv zwart zijn...
			...
			if(start->info){
				cout << "breedte " << breedte << ", hoogte " << hoogte << endl;
				if(Rand.TelAantOpl == 'n')
		...
		if ((teller + 1) % 10000 == 0){
			cout << endl << "tussenstand: " << (teller + 1) / 1;
			drukaf();
		}//if
		teller++;
	}//while
